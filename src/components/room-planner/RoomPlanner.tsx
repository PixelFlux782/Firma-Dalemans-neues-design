...
type PlannerValues = {
  ...
  doorEnabled: boolean; doorWall: "front" | "back" | "left" | "right"; doorPosition: number; doorWidth: number;
  obstacleEnabled: boolean; obstacleX: number; obstacleY: number; obstacleWidth: number; obstacleDepth: number;
};
...
const initialValues: PlannerValues = {
  ...
  doorEnabled: false, doorWall: "back", doorPosition: 5.5, doorWidth: 1,
  obstacleEnabled: false, obstacleX: 1, obstacleY: 8, obstacleWidth: 1, obstacleDepth: 1,
};
...
function ChairModel({ position, width, depth }: { position: [number, number, number]; width: number; depth: number }) {
  ...
  const chair = useMemo(() => {
    ...
    clone.scale.set(widthScale, widthScale, depth / Math.max(size.z, 0.001));
...
function RoomScene({ values, positions }: { values: PlannerValues; positions: [number, number, number][] }) {
  ...
  return <>
    ...
    <Suspense fallback={null}>{positions.map((position, index) => <ChairModel key={index} position={position} width={values.chairWidth} depth={values.chairDepth} />)}</Suspense>
...
function calculateLayout(values: PlannerValues) {
  ...
  const doorClearance = values.doorEnabled ? getDoorGeometry(values).clearance : null;
  const obstacle = values.obstacleEnabled ? { minX: values.obstacleX - values.obstacleWidth / 2, maxX: values.obstacleX + values.obstacleWidth / 2, minZ: values.obstacleY - values.obstacleDepth / 2, maxZ: values.obstacleY + values.obstacleDepth / 2 } : null;
...
export default function RoomPlanner() {
  ...
  return <div className="grid gap-6 lg:grid-cols-[18rem_minmax(0,1fr)]">
    <aside className="premium-card h-fit p-5 sm:p-6">
      ...
      <fieldset className="mt-6 border-t border-premium-beige pt-5"><legend className="font-semibold text-premium-ink">Tür</legend><label className="mt-3 flex items-center gap-2 text-sm"><input type="checkbox" checked={values.doorEnabled} onChange={(event) => setValues((current) => ({ ...current, doorEnabled: event.target.checked }))} />Tür aktiv</label>{values.doorEnabled ? <div className="mt-4 grid gap-4"><label className="grid gap-1.5 text-sm font-medium"><span>Wand</span><select aria-label="Wand" value={values.doorWall} onChange={(event) => setValues((current) => ({ ...current, doorWall: event.target.value as PlannerValues["doorWall"] }))} className="min-h-11 rounded-xl border border-premium-beige bg-white/80 px-3"><option value="front">vorne</option><option value="back">hinten</option><option value="left">links</option><option value="right">rechts</option></select></label><NumberInput label="Türposition" value={values.doorPosition} step={0.1} onChange={(doorPosition) => setValues((current) => ({ ...current, doorPosition }))} /><NumberInput label="Türbreite" value={values.doorWidth} step={0.1} onChange={(doorWidth) => setValues((current) => ({ ...current, doorWidth }))} /></div> : null}</fieldset>
      <fieldset className="mt-6 border-t border-premium-beige pt-5"><legend className="font-semibold text-premium-ink">Hindernis</legend><label className="mt-3 flex items-center gap-2 text-sm"><input type="checkbox" checked={values.obstacleEnabled} onChange={(event) => setValues((current) => ({ ...current, obstacleEnabled: event.target.checked }))} />Hindernis aktiv</label>{values.obstacleEnabled ? <div className="mt-4 grid gap-4"><NumberInput label="Hindernis X-Position" value={values.obstacleX} step={0.1} onChange={(obstacleX) => setValues((current) => ({ ...current, obstacleX }))} /><NumberInput label="Hindernis Y-Position" value={values.obstacleY} step={0.1} onChange={(obstacleY) => setValues((current) => ({ ...current, obstacleY }))} /><NumberInput label="Hindernisbreite" value={values.obstacleWidth} step={0.1} onChange={(obstacleWidth) => setValues((current) => ({ ...current, obstacleWidth }))} /><NumberInput label="Hindernistiefe" value={values.obstacleDepth} step={0.1} onChange={(obstacleDepth) => setValues((current) => ({ ...current, obstacleDepth }))} /></div> : null}</fieldset>
...
function NumberInput({ label, value, step, onChange }: { label: string; value: number; step: number; onChange: (value: number) => void }) {
  return <label className="grid gap-1.5 text-sm font-medium text-premium-charcoal"><span>{label}</span><span className="relative"><input type="number" step={step} value={value} onChange={(event) => onChange(Number(event.target.value))} className="min-h-11 w-full rounded-xl border border-premium-beige bg-white/80 px-3 pr-9 text-premium-ink outline-none transition focus:border-premium-sand focus:ring-2 focus:ring-premium-sand/30" /><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-premium-muted">m</span></span></label>;
}
